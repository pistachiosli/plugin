...
(function(){
    ...
    Module.prototype.render = function(container,data){
        ...
        //触发首次渲染事件
        // if(firstRender && DD.isFunction(me.onFirstRender)){
        //     me.onFirstRender.call(me.model);
        // }
    ...
    DD.assign(Module,{
        ...
        create:function(config){
            ...
            if(!DD.isEmpty(config.className)){
                ...
                if(cls !== undefined){
                    param = Object.assign({},cls,config);
                }
            }else{
                param = config;
            }
		...
		renderView:function(view,module,renderData){
			...
			function renderDom(node,isRoot){
	            ...
	            if(node.$isView){
	                ...
	                if(model.data === undefined || model.data.$changed || node.$forceRender || module.forceRender){
	        			if(DD.isEl(node)){
	                		//指令表达式处理
	                        var directives = [];
